...
lora_weights = [
    f'models/textural_inversion_classic_{SCENE}_lr{LEARNING_RATE}_alpha1.0_rank4_noxattn/textural_inversion_classic_{SCENE}_lr{LEARNING_RATE}_alpha1.0_rank4_noxattn_{checkpoint}steps.pt' for checkpoint in range(1000, 40000, 1000)
]
output_dir = f"output/textural_inversion/raw/textural_inversion_classic_{SCENE}_lr{LEARNING_RATE}_alpha1.0_rank4_noxattn"
        ...
        for _ in range(num_images_per_prompt):
            ...
            for lora_id, lora_weight in enumerate(lora_weights):
                ...
                if 'alpha1' in lora_weight:
                    alpha = 1.0
                network = TexturalInversionNetwork(learnable_size = 1).to(device, dtype=weight_dtype)
                        ...
                        for guidance_scale in guidance_scales:
                           ...
                           #text_embeddings = torch.cat([uncond_embeddings, text_embeddings])
                            ...
                            for t in tqdm(noise_scheduler.timesteps):
                                ...
                                global_token = network.get_global_token(0).to(device)[None]
                                ...
                                with torch.no_grad():
                                    embed_mask = torch.zeros_like(ori_text_embeddings)
                                    embed_mask[:, token_index] = torch.ones_like(embed_mask[:, token_index])

                                new_text_embeddings = ori_text_embeddings * (1 - embed_mask) + global_token.expand(-1, 77, -1) * embed_mask
                                


                                
                                text_embeddings = torch.cat([ori_uncond_embeddings, new_text_embeddings])
