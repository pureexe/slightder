...
from pure_util.lora_global_adapter import LoRAGlobalMultiScaleAdapter
...
def train(
    ...
    dataset = ImageAxis3Dataset(root_dir=folder_main, image_size = (image_size,image_size))
...
#                 1, 25, (1,)
        ...
        network.set_lora_slider(scale=batch['coeff'][...,:2])
        with network:
...
def main(args):
    ...
    if args.stylecheck is not None:
        ...
        for i in range(int(check[0]), int(check[1])):
            ...
            train(config=config, prompts=prompts, device=device, folder_main = folder_main, prompt_file = args.prompt_file, image_size = args.image_size)
    else:
        train(config=config, prompts=prompts, device=device, folder_main = args.folder_main, folders = folders, scales = scales, prompt_file = args.prompt_file, image_size = args.image_size)
