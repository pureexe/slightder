...
from pure_util.lora_global_adapter import LoRAGlobalAdapter
...
def train(
    ...
    prompt_pairs: list[PromptEmbedsPair] = [] #TOTAL LENGHT 1 (depend on config file)
...
#                 1, 25, (1,)
            ...
            latent1 = torch.load(f'{folder_latent}/{folder1}/last_hidden_states.pt')  #[1,1024,7,7]
        ...
        low_text_embedding = train_util.concat_embeddings(
            torch.cat([prompt_pair.unconditional, token1], axis=1), #CONCAT TOKEN
            torch.cat([prompt_pair.neutral, token1], axis=1),
...
def main(args):
    ...
    if args.stylecheck is not None:
        ...
        for i in range(int(check[0]), int(check[1])):
            ...
            train(config=config, prompts=prompts, device=device, folder_main = folder_main, folder_latent = args.folder_latent)
    else:
        train(config=config, prompts=prompts, device=device, folder_main = args.folder_main, folder_latent = args.folder_latent, folders = folders, scales = scales)
