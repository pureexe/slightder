...
#                 1, 25, (1,)
            ...
            cond_img1 = Image.open(f'{folder_condition}/{folder1}/{ims[random_sampler]}')#.resize((256,256))
            cond_img2 = Image.open(f'{folder_condition}/{folder2}/{ims[random_sampler]}')#.resize((256,256))

            # compute conditional image 2
            convnext_inputs = convnext_processor(cond_img2, return_tensors="pt")
            ...
            lora_scale2 = network.conv_head(convnext_last_hidden_states.view(-1, 1024 * 7 * 7))

            # compute conditional image 1
            convnext_inputs = convnext_processor(cond_img1, return_tensors="pt")
            convnext_inputs = {k: v.to("cuda") for k, v in convnext_inputs.items()}
            convnext_outputs = convnext_model(**convnext_inputs)
            convnext_last_hidden_states = convnext_outputs.last_hidden_state
            lora_scale1 = network.conv_head(convnext_last_hidden_states.view(-1, 1024 * 7 * 7))
        ...
        network.set_lora_slider(scale=lora_scale2)
        with network:
        ...
        #network.set_lora_slider(scale=-scale_to_look)
        network.set_lora_slider(scale=lora_scale1)
        with network:
...
def main(args):
    ...
    if args.stylecheck is not None:
        ...
        for i in range(int(check[0]), int(check[1])):
            ...
            folder_condition = args.folder_condition+ f'{i}'
            config.save.name = f'{os.path.basename(folder_main)}'
            config.save.name += f'_alpha{args.alpha}'
            config.save.name += f'_rank{config.network.rank }'
            config.save.path = f'models/{config.save.name}'
            train(config=config, prompts=prompts, device=device, folder_main = folder_main, folder_condition=folder_condition)
    else:
        train(config=config, prompts=prompts, device=device, folder_main = args.folder_main, folder_condition=args.folder_condition, folders = folders, scales = scales)
